...
# ─────────────────────────────────────────────
# PCLOUD AUTH  —  email + password  →  token
# ─────────────────────────────────────────────
def pcloud_login(username: str, password: str) -> dict:
    """
    Digest-auth login (SHA1) against both US and EU endpoints.
    Returns: { token, eu, email, quota, usedquota }
    Raises RuntimeError on bad credentials or network failure.
    """
    for eu in [False, True]:
        base = "https://eapi.pcloud.com" if eu else "https://api.pcloud.com"
        try:
            # Step 1 — get digest challenge
            r1 = requests.get(
                f"{base}/userinfo",
                params={"getauth": 1, "logout": 1, "username": username, "password": "x"},
                timeout=15,
            )
            d1 = r1.json()
        except Exception:
        ...
        digest   = d1["digest"]
        sha_user = hashlib.sha1(username.lower().encode()).hexdigest()
        pw_hash  = hashlib.sha1((password + sha_user + digest).encode()).hexdigest()
        ...
        if d2.get("result") == 0 and "token" in d2:
            return {
                "token":     d2["token"],
                "eu":        eu,
                "email":     d2.get("email", username),
                "quota":     d2.get("quota", 0),
                "usedquota": d2.get("usedquota", 0),
            }

        # Wrong password (result 2000) — no point trying EU
...
def pcloud_get_file_link(token, file_id, eu=False):
    d = requests.get(f"{_base(eu)}/getfilelink",
                     params={"access_token": token, "fileid": file_id}, timeout=30).json()
        ...
        if st.button("Đăng nhập", type="primary", use_container_width=True):
            if not all([email, password, groq_key]):
                ...
                with st.spinner("Đang xác thực…"):
                    try:
                        sess = pcloud_login(email, password)
                        sess["groq_key"] = groq_key
...
2. Sign up miễn phí — không cần thẻ
...
with c1:
    ...
    st.markdown(
        ...
        f'pCloud {"EU 🇪🇺" if eu else "US 🇺🇸"} &nbsp;·&nbsp; '
        f'{used:.1f} / {total:.0f} GB đã dùng</div>',
        unsafe_allow_html=True,
    )
with c2:
...
with left:
    ...
    folder_path = st.text_input("Thư mục pCloud", value="/",
                                 help="Quét đệ quy toàn bộ sub-folder")
    ...
    if "videos" in st.session_state:
        ...
        if not vids:
            ...
            if selected:
                ...
                if total_mb > 1500:
                    st.warning("⚠️ Dung lượng lớn — kiểm tra giới hạn disk Streamlit Cloud.")
