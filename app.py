...
def process_dubbing(sess, groq_key, video_info, lang_pair_cfg, voice, log_ph, prog_ph,
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # 4. TTS with edge-tts
        log(f"🔊 Generating TTS with voice: {VOICE_LABELS.get(voice, voice)}…")
        try:
...
# ════════════════════════════════════════════════
# TAB 6: DỊCH & DUBBING — Multi-Engine TTS
# ════════════════════════════════════════════════
with tab_dub:
    ...
    with ld:
        ...
        st.caption("Groq Whisper → Llama dịch → TTS engine → FFmpeg → Dropbox")
        ...
        if engine_key == "edge_tts":
            ...
            vidx  = st.selectbox("Chọn giọng", range(len(vlbls)),
                                  format_func=lambda i: vlbls[i], key="edge_voice")
            selected_voice = avail[vidx]
            with st.expander("🎛️ Điều chỉnh Edge-TTS", expanded=True):
                r = st.slider("Rate (%)", -50, 50, -10, 5,
                              help="Khuyến nghị -10% đến -20% để tránh nói quá nhanh")
                rate_str = f"{'+' if r>=0 else ''}{r}%"
                v = st.slider("Volume (%)", -50, 50, 0, 5)
                volume_str = f"{'+' if v>=0 else ''}{v}%"
                p = st.slider("Pitch (Hz)", -20, 20, 0, 2)
                pitch_str = f"{'+' if p>=0 else ''}{p}Hz"
                st.caption(f"Rate: `{rate_str}` · Vol: `{volume_str}` · Pitch: `{pitch_str}`")

        elif engine_key == "gtts":
            st.info("gTTS dùng giọng Google mặc định cho từng ngôn ngữ — không chọn giọng cụ thể.")
            st.caption(f"Ngôn ngữ output: **{GTTS_LANG.get(tgt_lang,'en')}**")
            selected_voice = GTTS_LANG.get(tgt_lang, "en")

        elif engine_key == "pyttsx3":
            st.info("pyttsx3 dùng giọng hệ thống (offline). Chất lượng phụ thuộc vào OS.")
            with st.expander("🎛️ Điều chỉnh pyttsx3", expanded=True):
                rate_wpm  = st.slider("Tốc độ đọc (WPM)", 80, 300, 150, 10,
                                       help="Words per minute. 150 = bình thường")
                vol_float = st.slider("Âm lượng", 0.0, 1.0, 0.9, 0.05)
                st.caption(f"Rate: **{rate_wpm} WPM** · Volume: **{vol_float:.0%}**")
            ...
            if not espeak_ok:
                st.error(
                    "⚠️ **espeak-ng chưa được cài trên server này.**\n\n"
                    "Trên Streamlit Cloud, thêm dòng `espeak-ng` vào file `packages.txt` "
                    "rồi redeploy. File `packages.txt` đã có sẵn trong repo của bạn."
                )
            else:
                st.success("✅ espeak-ng đã sẵn sàng!")
            st.info("eSpeak NG — siêu nhẹ, chạy offline, giọng tổng hợp cơ bản.")
            with st.expander("🎛️ Điều chỉnh eSpeak", expanded=True):
                espeak_speed = st.slider("Tốc độ (words/min)", 80, 400, 175, 10)
                espeak_pitch = st.slider("Pitch (0-99)", 0, 99, 50, 5)
                st.caption(f"Speed: **{espeak_speed}** · Pitch: **{espeak_pitch}**")
                st.markdown(f"Ngôn ngữ: `{ESPEAK_LANG.get(tgt_lang,'en')}`")
            selected_voice = ESPEAK_LANG.get(tgt_lang, "en")

        # ── Preview ───────────────────────────────
        st.markdown("**🎧 Nghe thử giọng**")
        prev_map = {"vi":"Xin chào, đây là giọng đọc tiếng Việt.",
                    "en":"Hello, this is a test of the selected voice.",
                    "zh":"你好，这是语音测试。"}
        preview_txt = st.text_input("Văn bản thử giọng",
        ...
        # ── Fit mode ──────────────────────────────
        st.markdown("**⚙️ Khớp thời lượng**")
        fit_mode = st.radio("Cách khớp TTS với segment gốc:",
        ...
        with st.expander("📊 So sánh các TTS Engine"):
            st.markdown("""
| Engine | Chất lượng | Offline | Tiếng Việt | Rate/Pitch | Tốc độ |
|--------|-----------|---------|-----------|-----------|--------|
| 🔵 **Edge-TTS** | ⭐⭐⭐⭐⭐ | ❌ | ✅ tốt | ✅ | Nhanh |
| 🟢 **gTTS** | ⭐⭐⭐⭐ | ❌ | ✅ tốt | ❌ | Nhanh |
| 🟡 **pyttsx3** | ⭐⭐ | ✅ | ⚠️ phụ OS | Rate | Rất nhanh |
| 🔴 **eSpeak** | ⭐⭐ | ✅ | ✅ cơ bản | ✅ | Rất nhanh |
            """)
            st.caption("Khuyến nghị: **Edge-TTS** cho tiếng Việt · **gTTS** khi Edge không hoạt động")
    ...
    with rd:
        ...
        if "dub_queue" in st.session_state:
            ...
            for i, video in enumerate(dq["videos"]):
                ...
                st.caption(
                    f"🌐 {dq['lang_label']} · {dq['engine_label']} · "
                    f"🔊 {VOICE_LABELS.get(dq['voice'], dq['voice'])}"
                )
            ...
            <div style="text-align:center;padding:4rem 1rem;color:#555">
                ...
                <div style="margin-top:1rem;font-size:1.05rem">
                    Chọn engine · cặp ngôn ngữ · giọng đọc<br>
                    nghe thử · chọn video<br>
                    nhấn <b>Bắt đầu Dịch & Lồng tiếng</b>
                </div>
                <div style="margin-top:1rem;font-size:.82rem;color:#444">
                    🔵 Edge-TTS · 🟢 gTTS · 🟡 pyttsx3 · 🔴 eSpeak
                </div>
