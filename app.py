            ...
            if d.get("result") == 0:
                ...
                if token:
                    ...
                    for param in ["auth","access_token"]:
                        v = requests.get(f"{base}/userinfo",params={param:token},timeout=10).json()
                        if v.get("result") == 0:
                            return {"token":token,"token_param":param,
                                    "eu":base.startswith("https://eapi"),
                                    "email":v.get("email",""),"quota":v.get("quota",0),
                                    "usedquota":v.get("usedquota",0)}
...
with st.expander("📁 Quét video từ pCloud",expanded=True):
    ...
    if scan_btn:
        with st.spinner("Đang quét…"):
            try:
                # Verify token still valid first
                token_check = requests.get(
                    f"{_base(_eu(sess))}/userinfo",
                    params=_auth(sess), timeout=10).json()
                if token_check.get("result") != 0:
                    st.error("❌ Token hết hạn — vui lòng đăng xuất và đăng nhập lại.")
                    st.stop()
