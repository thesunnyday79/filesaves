...
def add_logo_to_video(video_path: str, logo_path: str, output_path: str,
    ...
    # Build filter:
    # [1]scale + colorchannelmixer for alpha → overlay on [0]
    alpha_filter = f"colorchannelmixer=aa={opacity:.3f}"

    # Check if logo has alpha channel (PNG) — use format=rgba
    logo_ext = Path(logo_path).suffix.lower()
    if logo_ext == ".png":
        scale_filter = f"scale={logo_width}:-1,format=rgba,{alpha_filter}"
    else:
        # JPEG — add alpha via geq
        scale_filter = (
            f"scale={logo_width}:-1,format=rgba,"
            f"geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':a='{int(opacity*255)}'"
        )
    ...
    log_fn(f"   FFmpeg filter: {filter_complex[:80]}…")
    r = subprocess.run(cmd, capture_output=True, text=True)
    if r.returncode != 0:
...
with tab_logo:
    ...
    with left_l:
        ...
        logo_width = st.slider(
            "Chiều rộng logo (px)", 50, 500, 150, 10,
