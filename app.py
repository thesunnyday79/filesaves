...
def tts_segment(voice, text, out_path, rate="+0%"):
    ...
    try:
        ...
        loop.run_until_complete(_tts_segment(voice, text, out_path, rate=rate))
    except RuntimeError:
        asyncio.run(_tts_segment(voice, text, out_path, rate=rate))
...
def build_dubbed_audio(segments, translated_texts, voice, video_duration, tmp_dir, log_fn):
    ...
    2. Speed-adjust TTS to fit within segment duration (atempo)
    ...
    for i, (seg, text) in enumerate(zip(segments, translated_texts)):
        ...
        # Calculate speed ratio to fit TTS into segment duration
        # clamp atempo between 0.5 and 3.0 (FFmpeg limits)
        ...
        if abs(ratio - 1.0) < 0.05:
            ...
            # Handle ratio > 2.0 by chaining atempo filters
            if ratio <= 2.0:
                atempo = f"atempo={ratio:.3f}"
            else:
                # chain: atempo=2.0,atempo=ratio/2
                r2 = ratio / 2.0
                atempo = f"atempo=2.0,atempo={r2:.3f}"
            subprocess.run([
                "ffmpeg","-y","-i",tts_raw,
                "-af",atempo,
...
with tab_dub:
    ...
    with ld:
        ...
        st.caption("Groq Whisper transcribe → Llama dịch → Edge-TTS lồng tiếng → Dropbox")
        ...
        voice_labels = [VOICE_LABELS.get(v, v) for v in available_voices]
        voice_idx = st.selectbox("🔊 Giọng đọc", range(len(voice_labels)),
        ...
        <div style="background:#1e1e2e;border-left:4px solid #6C63FF;
            ...
            <b style="color:#cdd6f4">🤖 Pipeline</b><br>
            <small style="color:#888">
            1. Groq Whisper transcribe ({lang_pair_label.split("→")[0].strip()})<br>
            2. Llama 3.3-70B dịch sang {lang_pair_label.split("→")[1].strip()}<br>
            3. Edge-TTS tạo audio từng segment<br>
            4. FFmpeg ghép audio vào đúng timestamp<br>
            5. Upload video mới lên Dropbox
            </small>
        </div>
        """, unsafe_allow_html=True)

        # Speed note
        st.info("⚡ Edge-TTS tự động điều chỉnh tốc độ để khớp với thời lượng từng câu gốc.")
        ...
        if not st.session_state.get("videos"):
            ...
            if selected_d:
                ...
                est_min = total_mb / 50  # rough estimate
                st.caption(f"⏱️ Ước tính: ~{est_min:.0f}–{est_min*2:.0f} phút xử lý")
    ...
    with rd:
        ...
        if "dub_queue" in st.session_state:
            ...
            for i, video in enumerate(dq["videos"]):
                ...
                st.caption(f"🌐 {dq['lang_label']} · 🔊 {VOICE_LABELS.get(dq['voice'], dq['voice'])}")
                ph_prog = st.empty()
                ph_log  = st.empty()
            ...
            if ok_cnt == len(dq["videos"]):
                st.balloons()
                st.success(f"🎉 Xong! {ok_cnt}/{len(dq['videos'])} video đã được lồng tiếng & upload Dropbox.")
            else:
            ...
            <div style="text-align:center;padding:4rem 1rem;color:#555">
                ...
                <div style="margin-top:1rem;font-size:1.05rem">
                    Chọn cặp ngôn ngữ · giọng đọc<br>
                    chọn video · nhấn <b>Bắt đầu Dịch & Lồng tiếng</b>
                </div>
                <div style="margin-top:1rem;font-size:.82rem;color:#444">
                    Trung→Việt · Trung→Anh · Anh→Việt<br>
                    Edge-TTS miễn phí · tự khớp timestamp
                </div>
