...
EDGE_VOICES = {
    "vi": [
        "vi-VN-HoaiMyNeural",     # Female – smooth
...
VOICE_LABELS = {
    "vi-VN-HoaiMyNeural":  "Hoài My (Nữ - Việt)",
    "vi-VN-NamMinhNeural": "Nam Minh (Nam - Việt)",
...
async def _tts_segment(voice, text, out_path, rate="+0%", volume="+0%", pitch="+0Hz"):
    """Generate TTS audio for one segment using edge-tts."""
    import edge_tts
    comm = edge_tts.Communicate(text, voice, rate=rate, volume=volume, pitch=pitch)
    await comm.save(out_path)

def tts_segment(voice, text, out_path, rate="+0%", volume="+0%", pitch="+0Hz"):
    ...
    try:
        ...
        loop.run_until_complete(_tts_segment(voice, text, out_path,
                                              rate=rate, volume=volume, pitch=pitch))
    except RuntimeError:
        asyncio.run(_tts_segment(voice, text, out_path,
                                  rate=rate, volume=volume, pitch=pitch))
...
def build_dubbed_audio(segments, translated_texts, voice, video_duration, tmp_dir, log_fn,
    ...
    for i, (seg, text) in enumerate(zip(segments, translated_texts)):
        ...
        try:
            tts_segment(voice, text, tts_raw, rate=rate, volume=volume, pitch=pitch)
        except Exception as e:
...
with tab_dub:
    ...
    with ld:
        ...
        st.caption("Groq Whisper → Llama dịch → Edge-TTS → FFmpeg → Dropbox")
        ...
        available_voices = EDGE_VOICES.get(tgt_lang, EDGE_VOICES["vi"])
        voice_labels_list = [VOICE_LABELS.get(v, v) for v in available_voices]
        voice_idx = st.selectbox("Chọn giọng", range(len(voice_labels_list)),
                                  format_func=lambda i: voice_labels_list[i],
                                  key="dub_voice_sel")
        selected_voice = available_voices[voice_idx]

        # ── Custom voice settings ──────────────────
        with st.expander("🎛️ Tuỳ chỉnh giọng nói", expanded=True):
            st.markdown("**Tốc độ đọc (Rate)**")
            st.markdown("**Tốc độ đọc (Rate)**")
            rate_pct = st.slider(
                "Rate (%)", min_value=-50, max_value=50, value=-10, step=5,
                help="0% = bình thường · -20% = chậm · +20% = nhanh. Khuyến nghị: -10% đến -20%"
            )
            rate_str = f"{'+' if rate_pct >= 0 else ''}{rate_pct}%"
            st.caption(f"Rate: `{rate_str}`")

            st.markdown("**Âm lượng (Volume)**")
            vol_pct = st.slider(
                "Volume (%)", min_value=-50, max_value=50, value=0, step=5,
                help="0% = bình thường · +20% = to hơn · -20% = nhỏ hơn"
            )
            vol_str = f"{'+' if vol_pct >= 0 else ''}{vol_pct}%"
            st.caption(f"Volume: `{vol_str}`")

            st.markdown("**Pitch (Cao độ giọng)**")
            pitch_pct = st.slider(
                "Pitch (Hz)", min_value=-20, max_value=20, value=0, step=2,
                help="0 = bình thường · +10 = cao hơn · -10 = trầm hơn"
            )
            pitch_str = f"{'+' if pitch_pct >= 0 else ''}{pitch_pct}Hz"
            st.caption(f"Pitch: `{pitch_str}`")

            st.markdown("**Tốc độ ghép audio**")
            fit_mode = st.radio(
                "Cách khớp thời lượng TTS với segment gốc:",
                ["🐢 Chậm lại nếu TTS ngắn hơn (tự nhiên nhất)",
                 "⚡ Luôn khớp đúng timestamp (có thể nhanh/chậm)",
                 "✂️ Cắt bỏ nếu TTS dài hơn"],
                index=0,
                help="Chọn cách xử lý khi thời lượng TTS không khớp với câu gốc"
            )

        # ── Preview voice ──────────────────────────
        st.markdown("**🎧 Nghe thử giọng**")
        preview_text_map = {
            "vi": "Xin chào, đây là giọng đọc tiếng Việt được tạo tự động.",
            "en": "Hello, this is an automatically generated English voice.",
            "zh": "你好，这是自动生成的中文语音。",
        }
        preview_text = st.text_input(
            "Văn bản thử giọng",
            value=preview_text_map.get(tgt_lang, "Hello, testing voice."),
        )
        if st.button("🎧 Nghe thử", use_container_width=True):
            with st.spinner("Đang tạo audio…"):
                try:
                    ...
                    tts_segment(selected_voice, preview_text, preview_path,
        ...
        <div style="background:#1e1e2e;border-left:4px solid #6C63FF;
            ...
            <b style="color:#cdd6f4">⚙️ Cài đặt hiện tại</b><br>
            <small style="color:#888">
            🌐 {lang_pair_label}<br>
            🔊 {VOICE_LABELS.get(selected_voice, selected_voice)}<br>
            ⏩ Rate: {rate_str} · 🔉 Vol: {vol_str} · 🎵 Pitch: {pitch_str}
            </small>
        </div>
        ...
        if not st.session_state.get("videos"):
            ...
            selected_d = [opts_d[k] for k in sel_d]

            if selected_d:
                total_mb = sum(v["size"]/1e6 for v in selected_d)
                st.info(f"Đã chọn **{len(selected_d)}** video · ~**{total_mb:.0f} MB**")
                est = total_mb / 50
                st.caption(f"⏱️ Ước tính: ~{est:.0f}–{est*2:.0f} phút")
        ...
        if "dub_queue" in st.session_state:
            ...
            for i, video in enumerate(dq["videos"]):
                ...
                st.caption(
                    ...
                    f"Rate {dq['rate']} · Vol {dq['volume']} · Pitch {dq['pitch']}"
                )
                ph_prog = st.empty()
                ph_log  = st.empty()
            ...
            <div style="text-align:center;padding:4rem 1rem;color:#555">
                ...
                <div style="margin-top:1rem;font-size:1.05rem">
                    Chọn ngôn ngữ · tuỳ chỉnh giọng<br>
                    nghe thử · chọn video<br>
                    nhấn <b>Bắt đầu Dịch & Lồng tiếng</b>
                </div>
                <div style="margin-top:1rem;font-size:.82rem;color:#444">
                    Trung→Việt · Trung→Anh · Anh→Việt<br>
                    Rate / Volume / Pitch tuỳ chỉnh · Preview trực tiếp
                </div>
