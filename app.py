...
# ─────────────────────────────────────────────
# PCLOUD AUTH  —  store credentials, re-auth on each API call
# ─────────────────────────────────────────────
def _get_fresh_token(username, password):
    """
    Always get a fresh token using plain username+password.
    Try US then EU. Returns (token, token_param, eu, base).
    """
    for eu, base in [(False,"https://api.pcloud.com"),(True,"https://eapi.pcloud.com")]:
        try:
            ...
            d = r.json()
            if d.get("result") == 0:
                token = d.get("token") or d.get("auth")
                if token:
                    # verify which param works
                    for param in ["auth","access_token"]:
                        v = requests.get(f"{base}/userinfo",
                                         params={param:token},timeout=10).json()
                        if v.get("result") == 0:
                            return token, param, eu, base, v
            if d.get("result") == 2000:
...
def pcloud_login(username, password):
    """Login and store credentials (not just token) in session."""
    token, param, eu, base, userinfo = _get_fresh_token(username, password)
    return {
        "username":    username,
        "password":    password,
        "token":       token,
...
def get_valid_token(sess):
    """
    Return a working token. If current token is expired, refresh it automatically.
    Updates sess in place.
    """
    # Quick check
    try:
        v = requests.get(f"{_base(_eu(sess))}/userinfo",
                         params={sess["token_param"]: sess["token"]},
                         timeout=10).json()
        if v.get("result") == 0:
            return sess["token"]
    except Exception:
        pass
    # Token expired — get a fresh one using stored credentials
    try:
        token, param, eu, base, _ = _get_fresh_token(sess["username"], sess["password"])
        sess["token"] = token
        sess["token_param"] = param
        sess["eu"] = eu
        sess["base"] = base
        return token
    except Exception as e:
        raise RuntimeError(f"❌ Không thể refresh token: {e}")
...
def _auth(sess):
    """Always return fresh auth params, auto-refresh if needed."""
    try: get_valid_token(sess)
...
def pcloud_list_folder(sess, folder_id=0, path=None):
    ...
    r = requests.get(f"{_base(_eu(sess))}/listfolder", params=p, timeout=30).json()
    # If still auth error, force refresh and retry once
    if r.get("result") in (2000, 1000):
        try:
            get_valid_token(sess)
            p = _auth(sess)
            if path: p["path"] = path
            else:    p["folderid"] = folder_id
            r = requests.get(f"{_base(_eu(sess))}/listfolder", params=p, timeout=30).json()
...
def pcloud_get_file_link(sess, file_id):
    ...
    if d.get("result") in (2000,1000):
        try: get_valid_token(sess); d = requests.get(f"{_base(_eu(sess))}/getfilelink",params={**_auth(sess),"fileid":file_id},timeout=30).json()
        except Exception: pass
...
def pcloud_upload_file(sess, folder_id, local_path, filename):
    with open(local_path,"rb") as f:
        r = requests.post(f"{_base(_eu(sess))}/uploadfile",
...
def collect_videos(sess, folder_id, path_prefix="/"):
    ...
    if res.get("result") != 0:
        err_msg = res.get("error", str(res.get("result","Unknown")))
        # If token expired, re-show the error clearly
        return [], f"Log in failed (token expired?) — thử đăng xuất và đăng nhập lại. Detail: {err_msg}"
    videos = []
