...
# Available SDXL-class models (free tier)
T2I_MODELS = {
    "FLUX.1-schnell (Nhanh, chất lượng cao)":       "black-forest-labs/FLUX.1-schnell",
    "Stable Diffusion XL (Cổ điển, đa dụng)":       "stabilityai/stable-diffusion-xl-base-1.0",
    "Realistic Vision (Ảnh thực tế)":                "SG161222/Realistic_Vision_V6.0_B1_noVAE",
    "DreamShaper XL (Sáng tạo, nghệ thuật)":         "Lykon/dreamshaper-xl-turbo",
    "Anime Pastel Dream (Anime/Illustration)":        "Habib91/Anime_Pastel_Dream",
}
...
def generate_images_hf(hf_token: str, model_id: str,
    ...
    """
    Call HF Inference API, return list of PIL image bytes.
    Generates n_images sequentially (free tier doesn't support batch).
    """
    ...
    for i in range(n_images):
        payload = {
            ...
            "parameters": {
                ...
                "seed":                 i * 1337 + 42,   # different seed per image
            },
        ...
        results.append(resp.content)   # raw PNG bytes
...
with tab_t2i:
    ...
    st.caption("Stable Diffusion · Hugging Face API miễn phí · Lưu thẳng vào pCloud")
    ...
    with col_set:
        ...
        hf_token = st.text_input(
            ...
            type="password",
            placeholder="hf_...",
            help="Miễn phí tại huggingface.co → Settings → Access Tokens (READ)",
        )
        ...
        prompt = st.text_area(
            "✏️ Prompt (mô tả ảnh)",
            placeholder="A serene mountain landscape at golden hour, cinematic lighting, 8k, photorealistic",
            height=100,
        )
        neg_prompt = st.text_area(
            "🚫 Negative Prompt (không muốn có gì)",
        ...
        with st.expander("⚙️ Cài đặt nâng cao"):
            guidance  = st.slider("Guidance Scale", 1.0, 15.0, 7.5, 0.5,
                                  help="Cao = bám prompt hơn, thấp = sáng tạo hơn")
            steps     = st.slider("Inference Steps", 10, 50, 28, 1,
                                  help="Nhiều bước = chất lượng cao hơn nhưng chậm hơn")

        # ── pCloud folder ────────────────────────
        st.markdown("---")
        st.markdown("**📁 Lưu ảnh vào pCloud**")

        # Reuse scanned videos folder or let user pick
        if "videos" in st.session_state and st.session_state["videos"]:
            first_video = st.session_state["videos"][0]
            default_fid = str(first_video["parentfolderid"])
        else:
            default_fid = "0"
        ...
        save_folder_id = st.text_input(
            ...
            value=default_fid,
            help="Folder ID (số) trong pCloud. 0 = root. Lấy từ URL pCloud.",
        )
        img_prefix = st.text_input(
            "Tiền tố tên file",
            value="ai_image",
            placeholder="ai_image",
            help="File sẽ được đặt tên: ai_image_001.png, ai_image_002.png…",
        )

        # ── Generate button ───────────────────────
        st.markdown("---")
        can_generate = bool(hf_token and prompt.strip())
        if not hf_token:
            st.warning("⚠️ Cần nhập Hugging Face Token")
        elif not prompt.strip():
            st.warning("⚠️ Cần nhập Prompt mô tả ảnh")
        ...
        with st.expander("📌 Lấy Hugging Face Token miễn phí"):
            st.markdown("""
1. Vào **[huggingface.co](https://huggingface.co)** → Sign up miễn phí
2. **Settings → Access Tokens → New token**
3. Chọn role: **Read** → Generate → Copy
4. Paste vào ô trên
            """)
    ...
    with col_prev:
        ...
        if gen_btn and can_generate:
            ...
            with st.spinner(f"🎨 Đang generate 2 ảnh với {model_label.split('(')[0].strip()}…"):
                progress_ph.progress(0.1, "Kết nối Hugging Face API…")
                try:
                    imgs_bytes = generate_images_hf(
                        hf_token=hf_token,
                        model_id=model_id,
                        prompt=prompt,
                    ...
                    progress_ph.progress(0.7, "Generate xong, đang upload pCloud…")
                except Exception as e:
                    st.error(f"❌ Generate lỗi: {e}")
                    imgs_bytes = []

            if imgs_bytes:
                # Show images side by side
                img_cols = st.columns(len(imgs_bytes))
                saved_keys = []
                ...
                for i, img_bytes in enumerate(imgs_bytes):
                    ...
                    with img_cols[i]:
                        st.image(img_bytes, caption=f"Ảnh {i+1} — {fname}",
                                 use_container_width=True)

                        # Upload to pCloud
                        r = upload_image_to_pcloud(sess, fid, img_bytes, fname)
                        if r.get("result") == 0:
                            st.success(f"✅ Đã lưu: {fname}")
                            success_uploads.append(fname)
                        else:
                            st.error(f"❌ Upload lỗi: {r.get('error','')}")

                progress_ph.progress(1.0, "Hoàn tất!")

                if success_uploads:
                    st.balloons()
                    status_ph.success(
                        f"🎉 {len(success_uploads)}/2 ảnh đã lưu vào pCloud!"
                    )
            ...
            <div style="text-align:center;padding:4rem 1rem;color:#555;
                ...
                <div style="margin-top:1rem;font-size:1.05rem">
                    Nhập prompt và nhấn<br><b>Generate 2 ảnh</b>
                </div>
                <div style="margin-top:1rem;font-size:0.82rem;color:#444">
                    FLUX.1 · SDXL · Realistic · Anime<br>
                    Lưu thẳng vào pCloud
                </div>
            </div>""", unsafe_allow_html=True)
        ...
        if st.session_state.get("t2i_history"):
            st.markdown("---")
            st.markdown("**📋 Lịch sử generate:**")
            for h in reversed(st.session_state["t2i_history"][-5:]):
                st.markdown(
                    f'<div class="video-card">' 
                    f'<small style="color:#6C63FF">{h["model"]} · {h["size"]}</small><br>' 
                    f'<b style="font-size:0.9rem">{h["prompt"]}{"..." if len(h["prompt"])==80 else ""}</b><br>' 
                    f'<small style="color:#a6e3a1">✔ {", ".join(h["files"])}</small>' 
                    f'</div>',
                    unsafe_allow_html=True,
                )
