...
def _pcloud_try_login(base: str, username: str, password: str):
    ...
    Returns session dict on success, None on wrong server, raises on bad password.
    """
    # Step 1 — get fresh digest (valid 30 s)
    r1 = requests.get(f"{base}/getdigest", timeout=15)
    d1 = r1.json()
    if d1.get("result") != 0 or "digest" not in d1:
        return None   # unexpected — try other datacenter

    digest = d1["digest"]

    # Step 2 — compute digest password
    #   passworddigest = SHA1( password + SHA1( lowercase(username) ) + digest )
    sha_user  = hashlib.sha1(username.lower().encode("utf-8")).hexdigest()
    pw_digest = hashlib.sha1(
        (password + sha_user + digest).encode("utf-8")
    ).hexdigest()
    ...
    # result 2010 = wrong server / account on other DC — let caller try EU
    return None
...
def pcloud_login(username: str, password: str) -> dict:
    ...
    for eu, base in [(False, "https://api.pcloud.com"),
        ...
        try:
            result = _pcloud_try_login(base, username, password)
        except RuntimeError:
            raise          # bad password — stop immediately
        except Exception:
            continue       # network issue — try next DC
    ...
    raise RuntimeError(
        "❌ Không kết nối được pCloud (đã thử US + EU). "
        "Kiểm tra lại mạng hoặc thử lại sau."
    )
...
def pcloud_get_file_link(token, file_id, eu=False):
    d = requests.get(
        f"{_base(eu)}/getfilelink",
        params={"access_token": token, "fileid": file_id},
        timeout=30,
    ).json()
...
with left:
    ...
    folder_path = st.text_input(
        ...
        help="Quét đệ quy toàn bộ sub-folder. VD: /Videos hoặc /Courses",
    )
