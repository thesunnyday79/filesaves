...
def _run_espeak(lang_code, text, out_path, speed=175, pitch=50, volume=100):
    ...
    try:
        ...
        spk.pitch = pitch
        spk.volume = volume
        wavdata = spk.synt(text)
        with open(wav_path, "wb") as f: f.write(wavdata)
    except ImportError:
        # fallback to system espeak
        subprocess.run(
            ["espeak-ng", "-v", lang_code, "-s", str(speed),
             "-p", str(pitch), "-a", str(volume),
             "-w", wav_path, text],
            capture_output=True
        )
    # convert wav to mp3
    subprocess.run(["ffmpeg","-y","-i",wav_path,out_path], capture_output=True)
