...
def get_corner_color_hex(img_path: str) -> str:
    """
    Read top-left 3×3 pixel average of logo to detect background color.
    Returns hex string like 'ffcccc'.
    Uses ffmpeg to extract raw pixels.
    """
    import struct
    # Extract 5x5 top-left corner as raw RGB
    cmd = [
        "ffmpeg", "-y", "-i", img_path,
        "-vf", "crop=5:5:0:0,format=rgb24",
        "-f", "rawvideo", "-"
    ]
    r = subprocess.run(cmd, capture_output=True)
    raw = r.stdout  # 5*5*3 = 75 bytes
    if len(raw) < 9:
        return "ffffff"  # fallback white
    # Average first 9 pixels (3 rows × 3 cols × 3 channels)
    r_avg = sum(raw[i*3+0] for i in range(9)) // 9
    g_avg = sum(raw[i*3+1] for i in range(9)) // 9
    b_avg = sum(raw[i*3+2] for i in range(9)) // 9
    return f"{r_avg:02x}{g_avg:02x}{b_avg:02x}"


def remove_bg_color(logo_path: str, out_path: str,
                    color_hex: str = "ffffff",
                    similarity: float = 0.25,
                    blend: float = 0.05):
    """
    Remove background color using FFmpeg colorkey filter.
    - color_hex: background color to remove (auto-detected from corner)
    - similarity: 0.0–1.0 — how close a pixel must be to match (0.25 = ±25%)
    - blend: soft edge blending
    """
    cmd = [
        "ffmpeg", "-y", "-i", logo_path,
        "-vf", (
            f"format=rgba,"
            f"colorkey=0x{color_hex}:{similarity:.2f}:{blend:.2f}"
        ),
        out_path,
    ]
    r = subprocess.run(cmd, capture_output=True, text=True)
    if r.returncode != 0:
        raise RuntimeError(f"Remove bg error:\n{r.stderr[-1000:]}")
...
def add_logo_to_video(video_path: str, logo_path: str, output_path: str,
    ...
    if remove_white:
        try:
            bg_color = get_corner_color_hex(logo_path)
            log_fn(f"   Auto-detected bg color: #{bg_color} — removing…")
            remove_bg_color(logo_path, logo_clean,
                            color_hex=bg_color,
                            similarity=0.30,
                            blend=0.05)
        except Exception as e:
            log_fn(f"   ⚠️ Bg removal failed ({e}), using original", "warn")
...
with tab_logo:
    ...
    with left_l:
        ...
        logo_width = st.slider(
            "Chiều rộng logo (px)", 10, 300, 30, 5,
        ...
        margin = st.slider(
            "Khoảng cách mép (px)", 0, 100, 0, 2,
            help="0 = sát góc hoàn toàn",
        )
        ...
        remove_white = st.toggle(
            ...
            value=True,
            help="Tự động phát hiện màu nền (góc trên trái) và xoá → trong suốt. Dùng cho logo có nền màu bất kỳ.",
        )
        white_thresh = 240  # kept for backward compat
