...
with tab_t2i:
    ...
    with col_set:
        ...
        hf_token = st.text_input(
            ...
            type="password",
            placeholder="hf_...",
            help="Fine-grained token có quyền Inference tại huggingface.co/settings/tokens",
        )
        ...
        with st.expander("⚙️ Cài đặt nâng cao"):
            ...
            n_per_prompt = st.radio(
                ...
                index=1,
                horizontal=True,
                help="2 ảnh = 2 seed khác nhau, chọn ảnh đẹp hơn",
            )
        ...
        # ── START button ──────────────────────────
        st.markdown("---")
        can_run = bool(hf_token and n_prompts > 0)
        if not hf_token:
            st.warning("⚠️ Cần nhập HF Token (Fine-grained + Inference)")
        elif n_prompts == 0:
            st.warning("⚠️ Chưa có prompt nào")

        start_batch = st.button(
            f"🎨 Generate {n_prompts} prompt × {n_per_prompt} ảnh = {n_prompts*n_per_prompt} ảnh",
            type="primary",
            use_container_width=True,
            disabled=not can_run,
        )

        with st.expander("📌 Lấy HF Token đúng cách"):
            st.markdown("""
1. Vào **[huggingface.co/settings/tokens](https://huggingface.co/settings/tokens)**
2. **"Create new token"** → chọn **`Fine-grained`**
3. Kéo xuống **"Inference"** → bật ✅ **"Make calls to Inference Providers"**
4. Create → Copy → Paste vào ô trên
            """)
