...
# ─────────────────────────────────────────────
# ── AI Horde — 100% free, no API key required ──
# Use anonymous key "0000000000" or register at aihorde.net for priority
HORDE_API      = "https://aihorde.net/api/v2"
HORDE_ANON_KEY = "0000000000"

T2I_MODELS = {
    "AlbedoBase XL (Chất lượng cao)":        "AlbedoBase XL (SDXL)",
    "Dreamshaper XL (Sáng tạo)":             "Dreamshaper XL 1.0",
    "SDXL (Stable Diffusion XL)":            "stable_diffusion_xl",
    "Realistic Vision (Ảnh thực tế)":        "Realistic Vision",
    "Anime Pastel Dream (Anime)":             "Anime Pastel Dream",
}
...
def generate_images(model_id: str,
                     ...
                     n_images: int = 2,
                     seed_base: int = 42,
                     horde_key: str = HORDE_ANON_KEY) -> list:
    """
    Generate images via AI Horde — 100% free, anonymous key works.
    Flow: POST /generate/async → poll /generate/check/{id} → GET /generate/status/{id}
    Returns list of raw PNG bytes.
    """
    import time, base64
    ...
    headers = {
        "apikey":       horde_key,
        "Content-Type": "application/json",
        "Client-Agent": "pcloud-autocaption:1.0:streamlit",
    }

    for i in range(n_images):
        seed = seed_base + i * 1337
        ...
        if resp.status_code not in (200, 202):
            raise RuntimeError(f"AI Horde submit error {resp.status_code}: {resp.text[:300]}")

        job_id = resp.json().get("id")
        if not job_id:
            raise RuntimeError(f"No job ID returned: {resp.text[:200]}")

        # Step 2: poll until done (max 5 min)
        deadline = time.time() + 300
        while time.time() < deadline:
            time.sleep(8)
            chk = requests.get(f"{HORDE_API}/generate/check/{job_id}",
            ...
            if chk_data.get("done"):
                break
            wait_time = chk_data.get("wait_time", "?")
            queue_pos = chk_data.get("queue_position", "?")
            # caller can use this info in logs if needed

        # Step 3: fetch result
        status = requests.get(f"{HORDE_API}/generate/status/{job_id}",
                              headers=headers, timeout=30)
        if status.status_code != 200:
            raise RuntimeError(f"AI Horde status error {status.status_code}: {status.text[:200]}")
        ...
        if img_url.startswith("data:image"):
            ...
            raise RuntimeError(f"Unexpected image format: {img_url[:80]}")
...
# ── Two feature tabs ───────────────────────────
tab_caption, tab_music, tab_shorts, tab_t2i = st.tabs(["🎬 Auto Caption", "🎵 Background Music", "✂️ YouTube Shorts", "🖼️ Text to Image"])
...
# ════════════════════════════════════════════════
# TAB 4: TEXT TO IMAGE  —  Batch Mode
# ════════════════════════════════════════════════
with tab_t2i:
    st.subheader("🖼️ Text to Image — Batch")
    st.caption("Dán nhiều prompt · AI tạo từng ảnh tuần tự · Lưu thẳng vào pCloud")

    col_set, col_prev = st.columns([1, 1.4], gap="large")

    with col_set:
        # ── HF Token ─────────────────────────────
        st.success("✅ AI Horde — Miễn phí hoàn toàn, dùng anonymous key!")
        ...
        width, height = SIZE_OPTIONS[size_label]
        ...
        with st.expander("⚙️ Cài đặt nâng cao"):
            n_per_prompt = st.radio(
                ...
                index=1,
                horizontal=True,
                help="2 ảnh = 2 seed khác nhau để chọn ảnh đẹp hơn",
            )
        ...
        neg_prompt = st.text_area(
            ...
            value="blurry, low quality, distorted, watermark, text, ugly, bad anatomy",
            height=68,
        )
        ...
        batch_text = st.text_area(
            "Danh sách prompts (1 dòng = 1 prompt)",
            "Danh sách prompts (1 dòng = 1 prompt)",
        ...
        # ── START button ──────────────────────────
        st.markdown("---")
        can_run = bool(n_prompts > 0)
        if n_prompts == 0:
            st.warning("⚠️ Chưa có prompt nào")

        start_batch = st.button(
            f"🎨 Generate {n_prompts} prompts × {n_per_prompt} ảnh = {n_prompts*n_per_prompt} ảnh",
            type="primary",
            use_container_width=True,
            disabled=not can_run,
        )

        with st.expander("ℹ️ Về AI Horde + tăng tốc độ"):
            st.markdown("""
**AI Horde** là mạng lưới GPU cộng đồng — hoàn toàn miễn phí:
- ✅ Không cần API key (dùng anonymous)
- ✅ Không giới hạn credits
- ✅ Nhiều model: SDXL, Realistic, Anime…
- ⚡ **Muốn nhanh hơn?** Đăng ký tại [aihorde.net](https://aihorde.net) (miễn phí) để nhận priority key
- Nhập key vào ô **AI Horde API Key** bên dưới để tăng tốc độ xử lý
            """)

        horde_key = st.text_input(
            "🔑 AI Horde API Key (tuỳ chọn)",
            value="",
            placeholder="Để trống = dùng anonymous (chậm hơn)",
            help="Đăng ký miễn phí tại aihorde.net để có key riêng, xử lý nhanh hơn",
        )
        effective_key = horde_key.strip() if horde_key.strip() else HORDE_ANON_KEY
    ...
    with col_prev:
        ...
        if start_batch and can_run:
            ...
            for pi, prompt in enumerate(raw_prompts):
                ...
                prompt_slots[pi].markdown(
                    f'<div class="log-box">⏳ [{pi+1}/{n_prompts}] {short_prompt}</div>',
                    unsafe_allow_html=True,
                )
                ...
                try:
                    ...
                    for ii, img_bytes in enumerate(imgs):
                        ...
                        if r.get("result") == 0:
                            ...
                            gallery_imgs.append((fname, img_bytes, prompt))
                            done_imgs += 1
                        else:
                            errors.append(f"Upload {fname}: {r.get('error','')}")

                    # Update prompt slot → success
                    files_str = ", ".join(saved_files)
                    prompt_slots[pi].markdown(
                        f'<div class="log-box log-success">✔ [{pi+1}/{n_prompts}] {short_prompt}<br>'
                    ...
                    err_msg = str(e)[:120]
                    prompt_slots[pi].markdown(
                        f'<div class="log-box log-error">✖ [{pi+1}/{n_prompts}] {short_prompt}<br>'
                ...
                pct = (pi + 1) / n_prompts
                overall_bar.progress(pct, text=f"{done_imgs} / {total_imgs} ảnh · Prompt {pi+1}/{n_prompts}")

                # Mini gallery (latest 6)
                if gallery_imgs:
                    recent = gallery_imgs[-6:]
                    gcols  = st.columns(min(3, len(recent)))
                    for gi, (gname, gbytes, gprompt) in enumerate(recent):
            ...
            if ok_count == n_prompts:
                ...
                overall_stat.success(
                    f"🎉 Hoàn tất! {done_imgs}/{total_imgs} ảnh đã upload pCloud."
                )
            else:
            ...
            <div style="text-align:center;padding:4rem 1rem;color:#555;
                ...
                <div style="margin-top:1rem;font-size:1.05rem">
                    ...
                    (mỗi dòng 1 prompt)<br>
                    rồi nhấn <b>Generate</b>
                </div>
                <div style="margin-top:1rem;font-size:0.82rem;color:#444">
                    Tool tự động xử lý từng prompt<br>
                    và lưu ảnh vào pCloud
                </div>
            </div>""", unsafe_allow_html=True)
        ...
        if st.session_state.get("t2i_history"):
            ...
            for h in reversed(st.session_state["t2i_history"][-5:]):
                st.markdown(
                    f'<div class="video-card">'
                    f'<small style="color:#6C63FF">{h["model"]} · {h["size"]}</small><br>'
